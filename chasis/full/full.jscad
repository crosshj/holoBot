...
function getParameterDefinitions() {
  return [
    ...
    { name: 'wallHeight', caption: 'Height:', type: 'float', initial: 40.0 },
...
function main(params){
    return union([
        domeMain(params).translate([0,0,params.wallHeight]),
        wallMain(params)
    ]);
}

// -----------------------------------------------------------
function wallMain(params){
    return union([
        wallQuarter(params),
        wallQuarter(params).rotateZ(90).translate([0,0,0]),
        wallQuarter(params).rotateZ(180).translate([0,0,0]),
        wallQuarter(params).rotateZ(270).translate([0,0,0]),
    ]);
}
...
function wallQuarter(params) {
   ...
   var b = difference([
    polygonPlate(boo),
    polygonPlate(koo),
    onlyShowAFourth(params.wallHeight).rotateZ(45),
   ]);
   ...
   return union([
       b,
       //wallmount(params.wallHeight).translate([54,-49,0]),
       //wallmount(params.wallHeight).translate([54,49,0]),
   ]).scale([1.085,1.085,1]);
}

// -----------------------------------------------------------
function domeMain(params){
    return union([
        quarterdome(params),
        quarterdome(params).rotateZ(90).translate([0,0,0]),
        quarterdome(params).rotateZ(180).translate([0,0,0]),
        quarterdome(params).rotateZ(270).translate([0,0,0]),
    ]);
}
